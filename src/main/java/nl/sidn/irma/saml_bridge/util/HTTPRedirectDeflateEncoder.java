...
public class HTTPRedirectDeflateEncoder implements SAMLMessageEncoder {
    ...
    protected String buildRedirectURL(final MessageContext messageContext, final String endpoint, final String message)
        ...
        // This is a copy of any existing allowed params that were preserved. Note that
        // they will not be signed.
        final List<Pair<String, String>> originalParams = new ArrayList<>(queryParams);

        // We clear here so that existing params will not be signed, but can still use
        // the URLBuilder#buildQueryString()
        // to build the string that will potentially be signed later. Add originalParms
        // back in later.
        queryParams.clear();
        ...
        final SignatureSigningParameters signingParameters = SAMLMessageSecuritySupport
        ...
        if (signingParameters != null && signingParameters.getSigningCredential() != null) {
            ...
            // Add original params to the beginning of the list preserving their original
            // order.
    ...
    /**
     * A subclass of {@link DeflaterOutputStream} which defaults in a no-wrap
